  ...
  def top = new MainFrame {
    ...
    val selections: Observable[String] = {
      button.clicks.flatMap {
        b => if (suggestionList.selection.items.size == 0) Observable.empty
          else Observable.just(suggestionList.selection.items.self: _*)
      }
    }
