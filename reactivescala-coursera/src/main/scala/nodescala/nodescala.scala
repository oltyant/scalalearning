...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    response.takeWhile(_ => token.nonCancelled) foreach( exchange.write(_))
    exchange.close()
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val respondSubscription = Future.run() { ct =>
      async {
    ...
    Subscription(listenerSubscription, respondSubscription)
  }
