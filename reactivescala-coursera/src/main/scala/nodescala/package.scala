...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match {
      case Nil => Future(Nil)
      case head :: tail => head.flatMap(x => all(tail).flatMap(xs => Future(x :: xs)))
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs.foreach {
        f => f.onComplete(p.tryComplete(_))
        //f => p.tryCompleteWith(f)
      }
      p.future
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val ts: CancellationTokenSource = CancellationTokenSource()
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    /** Returns the result of this future if it is completed now.
      ...
      */
    def now: T = {
      Try(Await.result(f, -1 nanos)) match {
        case Success(e) => e
        case Failure(ex: TimeoutException) => throw new NoSuchElementException
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete {
        case _ => p.tryComplete(Try(cont(f)))
      }
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete {
        case r => p.tryComplete(Try(cont(r)))
      }
      p.future
    }
