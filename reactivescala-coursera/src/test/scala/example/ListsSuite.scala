...
class ListsSuite extends FunSuite with Checkers {
  /**
  ...
  test("check sum of a List") {
    check {
      (l: List[Int]) => Lists.sum(l) == l.foldLeft(0)(_ + _)
    }
  }

  test("check max of List") {
    check {
      (l: List[Int]) => {
        try {
          Lists.max(l) == l.foldRight(l.head)(math.max)
        } catch {
          case e: NoSuchElementException if l.isEmpty => true
